<script lang="ts">
	/**
	 * Sitenin `content-dialog` şablonunu kullanan, banner + logo + Setsuki
	 * maskotlu ortak diyalog kabuğu.
	 *
	 * `AboutDialog.svelte` ile `UpdateDialog.svelte` bilerek AYNI bileşeni
	...
	/** Kapalıyken arka plana tıklama / Escape / X düğmesi devre dışı kalır —
	 * ör. bir indirme sürerken yanlışlıkla diyaloğu kapatmayı önlemek için. */
...
{#if open}
	...
	>
		<!--
			...
			dialog kutusunu VE kapatma düğmesini BİRLİKTE sarıyor. Yalnızca
			`.content-dialog`'un kendi geçişi olsaydı, düğme bir kardeş olarak
			dışarıda kalıp yalnızca üstteki `fade`'den opaklık alırdı — ikisi
			farklı hızda belirir, düğme dialogdan önce "zaten oradaymış" gibi
			görünürdü. Kütüphanenin kendi `ContentDialog`'u (takipçi/takip
			edilen modalındaki kapatma düğmesi) tam olarak bunu yapıyor:
			`content-dialog-container` (dialog + düğme) TEK geçişle birlikte
			beliriyor (bkz.
		...
		>
			<div class="content-dialog size-max svelte-f1dwd4" role="dialog" aria-modal="true">
					...
					<div id="main" class="fds-theme-dark svelte-cc3kyp">
						<div id="card" class="svelte-cc3kyp">
							...
							<div id="info" class="fds-theme-dark svelte-cc3kyp">
								<h4 class="text-block type-subtitle svelte-9tjxrp">{title}</h4>
								<span class="text-block type-caption text-tertiary svelte-9tjxrp">{subtitle}</span>
							</div>
						</div>
...
<style>
	...
	#content :global(a.button.style-hyperlink) {
		...
		text-align: center;
		transition:
			background-color var(--fds-control-fast-duration, 0.15s) ease,
	...
	#content :global(a.button.style-hyperlink:hover) {
	...
	#content :global(a.button.style-hyperlink:active) {
