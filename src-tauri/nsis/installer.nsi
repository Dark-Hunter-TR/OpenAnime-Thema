...
Function OaCustomPage
  ...
  ; Sayfa oluşturma
  !insertmacro MUI_HEADER_TEXT "${PRODUCTNAME} Kurulum Seçenekleri" "Lütfen yapmak istediğiniz işlemi seçin."
  ...
  ${NSD_CreateLabel} 0 0 100% 20u "${PRODUCTNAME}'i bilgisayarınıza kurmak veya mevcut kurulumu yönetmek üzeresiniz."
  Pop $R0
  ...
  ${If} $OaIsInstalled == 1
    ...
    ${NSD_CreateRadioButton} 0 40u 100% 12u "Temiz Kurulum (kayıtlı temalar ve ayarlar dahil her şeyi siler)"
    Pop $OaRadio2
    ${NSD_OnClick} $OaRadio2 OaOnRadioChange

    ${NSD_CreateRadioButton} 0 55u 100% 12u "${PRODUCTNAME}'i Kaldır"
    Pop $OaRadio3
...
Function OaOnInstFilesShow
  ...
  GetDlgItem $1 $0 1006
  SendMessage $1 ${WM_SETTEXT} 0 "STR:${PRODUCTNAME} kuruluyor... Lütfen bekleyin."
FunctionEnd
...
Function .onInit
  ${GetOptions} $CMDLINE "--auto-update" $R0
  ${IfNot} ${Errors}
    SetSilent silent
  ${EndIf}
...
!define OA_LATEST_MANIFEST_URL "https://github.com/Dark-Hunter-TR/OpenAnime-Thema/releases/latest/download/latest.json"
!define OA_RELEASE_DL_BASE "https://github.com/Dark-Hunter-TR/OpenAnime-Thema/releases/download"
...
Function CheckOnlineLatest
  ...
  Delete "$TEMP\oa_latest.json"
  nsExec::Exec '"$SYSDIR\curl.exe" -L -s -f -A "OpenAnime-Theme-Installer" --connect-timeout 5 --max-time 20 -o "$TEMP\oa_latest.json" "${OA_LATEST_MANIFEST_URL}"'
  Pop $0
  ${If} $0 != "0"
    Push "${PRODUCTNAME} kuruluyor... Lütfen bekleyin."
    Call OaSetInstText
    Return
  ${EndIf}
  ...
  StrCpy $8 "${PRODUCTNAME}_"      ; ${PRODUCTNAME}_
  StrCpy $8 "$8$OaOnlineVersion"   ; ${PRODUCTNAME}_<ver>
  StrCpy $9 "${OA_SETUP_SUFFIX}"   ; _<arch>-setup.exe
  StrCpy $8 "$8$9"                 ; ${PRODUCTNAME}_<ver>_<arch>-setup.exe
  ...
  Call OaSetInstText
  Delete "$TEMP\oa_latest_setup.exe"
  nsExec::Exec '"$SYSDIR\curl.exe" -L -s -f -A "OpenAnime-Theme-Installer" --connect-timeout 5 --max-time 300 -o "$TEMP\oa_latest_setup.exe" "$OaSetupUrl"'
  Pop $0
  ${If} $0 != "0"
    Delete "$TEMP\oa_latest_setup.exe"
    Push "${PRODUCTNAME} kuruluyor... Lütfen bekleyin."
